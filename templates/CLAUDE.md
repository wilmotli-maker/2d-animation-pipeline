...
- `pipeline review shots --slug <name> [--match <re>] [--exclude <re>] [--characters a,b] [--episode N,M] [--layout side-by-side|stacked] [--update] [--out <dir>]` · `pipeline review images --slug <name> [--match <re>] [--exclude <re>] [--characters a,b] [--sheets turnaround,pose,cycles] [--update] [--out <dir>]` — build/refresh a self-contained review page. Written to `<root>/web/<slug>/` by default (created if missing); `--out <dir>` picks another folder. Client-side filtering + side-by-side version comparison. Filters combine (intersection); `--exclude` drops ids/slugs matching a regex (e.g. `candidates|assembled`). Add `--folder <dir>` to build the page from a flat folder of `<shotId>-vNNN.ext` clips instead of the `shots/` tree. Vendored clips/images under `<slug>/assets/` are gitignored.
